...
- `scripts/template/__tests__/worktree.test.ts` — refusal (exit 7, no `devcontainer up`, no `docker exec`, no command), ready-only execution through the recorded container, the in-container no-op with an exit status preserved, option-parsing coverage for the new flag, and real-Git hook routing in both topologies including the graphify guard, the direct fallback, and a refused commit that does not land.
