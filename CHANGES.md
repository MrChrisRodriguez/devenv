...
**Changed files:**
- `scripts/worktree/doctor.sh` — the host-only, read-only diagnostic: argument parsing and the exit-2 refusal, `add_result`, `json_escape`, the human and JSON renderers, `--list-checks`, and the host prerequisite phase (`host.context`, the five role-named `host.command.*` checks, and `host.engine-daemon`).
...
- `scripts/template/__tests__/worktree.test.ts` — invalid-argument refusals, the probe-free inventory listing, the stable JSON document, the in-container refusal reduced to exactly one check, `--strict` changing the exit code with a deep-equal checks array, and a purity/parity probe proving the shared identity derivation matches the container CLI and writes nothing.
