...
- `scripts/template/__tests__/worktree.test.ts` — invalid-argument refusals, the probe-free inventory listing, the stable JSON document, the in-container refusal reduced to exactly one check, and `--strict` changing the exit code with a deep-equal checks array.
