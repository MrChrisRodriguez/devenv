...
## 2026-08-07 — Fix: record the released decision, and give the gate a reachable one

**Goal:** Finish the release gate's own runbook, and fix the reason it could not be finished. `v1.0.0` is tagged on the Stage 11 merge commit and the change is archived, so the last post-merge step is to flip `release.json#decision` from `candidate` to `released` and record the two CI signals the requirement names. Attempting it found the gate in a state with **no legal terminal value**, and `main` was already red because of it.

**The two refusals were mutually exclusive and both were reachable.** `reconcileDecision` refuses `released` while the planned tag does not exist — the rule that keeps a record from upgrading its own gate — and it refuses `candidate` once the tag *does* exist, so a tagged tree that still calls itself a candidate is a record that stopped describing its own repository. Both are correct. Together they make the flip mandatory rather than optional the moment `git push origin v1.0.0` lands, which is why `main` went red on the very next push: `release: v1.0.0 already exists but release.json still declares the candidate decision`, failing `template:validate`, the release gate and fourteen tests.

**And the `released` side could not be reached, because the exact-head signal anchored on `HEAD`.** `validateSignals` required a captured `pr-exact-head` signal's sha to equal `git rev-parse HEAD`. That is exactly right while a tree is under review — the tree being read must be the tree the green run belongs to — but the flip is a commit, a commit moves `HEAD`, and no commit can carry its own object id. There is no fixed point. A tree could therefore be refused as a candidate for having been tagged and refused as released for having been committed.

**The anchor is now the decision's own consequence, and the released one is the tag.** While the tree is a `candidate`, nothing changes: the exact-head sha must equal `HEAD`. Once it is `released`, the sha must name a commit the planned tag **contains** — `git merge-base --is-ancestor <sha> <tag>` — which is what the runbook actually did, since it tags the merge commit whose parent is the reviewed head. That is still a declaration checked against local Git objects and never a question asked of a Checks API, it is narrower than "some commit in this repository" rather than wider, and it is provable in both directions: the archive commit that landed *after* the tag is a real commit the tag does not contain, and it is refused.

**The signals themselves are now captured rather than pending.** `pr-exact-head` is run `31214545642` at `a8be2fd6…`, the affected-mode run on PR #38's exact head; `default-branch-full` is run `31215171505` at `0e170a0f…`, the full-mode push run on the merge commit, which is the signal Stage 11 recorded as impossible to have before a merge existed. Both were watched green by a human and are asserted here only against local Git objects, as designed.

**Two follow-on corrections came with it, both mechanical.** The archive commit added seven `renderPolicy: omit` paths under `openspec/changes/archive/**`, so every fixture's `omittedCount` rose by seven; `bun run template:release-sync` regenerated the three golden manifests and the declared counts, and `evidence/stage-11-release.json#release.goldenDigests` — which binds those files by sha256 precisely so a silent regeneration is impossible — was re-sealed to match. The record's own `decision` stays `candidate`: it describes the pull request that was audited, not the follow-up that flipped it.

**And Stage 11's L20 fix had a second site nobody found.** `openspec.test.ts` asserted this repository's change is ACTIVE in two places. The aggregate leg was made archive-tolerant in Stage 11 and the disposable-lifecycle leg was not, so it failed on `main` the moment the archive landed. It now counts the change exactly once across active ∪ archived, the same shape and for the same reason: asserting active fails after the archive, asserting archived fails inside the pull request that has not merged, and counting once is green on both sides while still catching a change the probe deleted.

**Downstream projects need none of this.** `release.json`, `release.schema.json`, the goldens and the release contract are template-only and are omitted from every render.
...
**`AGENTS.md` gains no new section, and that is a mechanical decision rather than a stylistic one.** `stripTemplateOnlyBlocks` matches a `#`-comment form, and in markdown `# template-only:start release` renders as an H1 — **template-only blocks do not work in markdown**. A `## Release Ownership` section would therefore ship into every generated project, describing a guard that is not there: an agent instruction for a capability the project does not have, which is exactly what the capability model forbids. What ships instead is a **completeness assertion** over the sections that already exist. `release.json#agentRuleSections` declares which `## …` section documents each of the fifteen `*:check` guards — a mapping that has to be declared because it is not derivable: fourteen guards map to `## … Ownership` sections, `rules:check` maps to `## Canonical Agent Rules`, and `worktree:check` is named in no line of `AGENTS.md` at all. For every render, the guard asserts that each `*:check` script that survived has a section that survived with it, so a capability whose guard ships without its documentation — or the reverse — is a refusal. The release gate's own scripts are refused *from* that mapping, which proves the template-only decision from the other side.

**Onboarding is corrected where it was wrong and extended where it was silent.** The entry point was already right — `README.md` and `README.template.md` both describe `bash scripts/worktree/up.sh` and the `--require-ready` exit 7, corrected when the isolated worktree runtime became the way to start a container. What neither document had was any account of what this repository actually is: `README.md` gains **Capabilities and Profiles**, naming all nineteen capabilities and the three committed fixture profiles and stating the two consequences that surprise people — a disabled capability leaves no residue, and ownership is declared rather than guessed — and **Validating the Template**, naming `template:validate`, the two `template:release-*` commands, the fifteen guards and the single required `ci-gate` status check. The eight host-prerequisite subsections are untouched: they were correct and long, and an unbounded rewrite in the release commit is the one change nobody can review against a spec clause.

**`docs/troubleshooting.md` is new, ships to every generated project, and exists to stop the same four things being diagnosed twice.** The `--require-ready` exit 7 that refuses a commit; a container rebuild after a change under `.devcontainer/` that looked cosmetic, because the definition fingerprint is over bytes rather than over meaning; two independent clones of one project colliding, which is a documented rule the runtime cannot enforce because from inside either checkout they look identical; and a freshly generated project reporting `fingerprint input bun.lock is missing`, which is correct by design. Each entry names the mechanism and says explicitly which "fix" would make things worse. The `bun.lock` entry sits inside a `codex_cloud` capability fence, because it names `cloud:check` — a residue token — and the anti-residue scan caught it in the `minimal` render before this paragraph was written.

**Ten acceptance items, seven re-measured live at this head.** A clean image build in 118.6 s against Stage 0's 312.33 s; a warm rebuild in 6.4 s against 17.08 s; two simultaneous isolated worktrees on disjoint ports with distinct manifests; a browser preflight passing in 171 ms inside the `development_browser` target; the OpenSpec lifecycle and the dependency guards. And a **fresh container lifecycle reaching ready in 124.1 s** — which is not an improvement but a **first**: Stage 0 recorded fresh startup and readiness as `"unavailable"` because neither isolated worktree ever completed one. The three inherited items name the record and the boundary they inherit from, and the guard prints them on success so nobody reads a green gate as a full re-run.

**The rollback is proved as two equal tree object ids.** A synthetic merge of this branch into its predecessor, a `git revert -m 1` of that merge, and the assertion that the reverted tree is byte-identical to the predecessor's — plus every added path present after the merge and gone after the revert, zero changes under the definition-fingerprint inputs, and zero lockfile movement. "We could revert this" is a claim; two equal tree ids are a fact.

**Downstream adoption: none, and that is the point.** `release.json`, `release.schema.json`, `fixtures/golden/**`, `scripts/template/release-contract.ts`, `scripts/template/validate-release.ts` and `scripts/template/sync-release-goldens.ts` are omitted from every render, the two package scripts are stripped by their `template:` prefix, and the workflow step is stripped with its `template-only` block. A generated project receives nothing from this stage and needs to do nothing. Nothing under `.devcontainer/**` changes and no container rebuild is required. `bun.lock` does not move: the guard is `node:*`, `./json-schema` and the existing renderer. Full reasoning, the decision table, all fifteen recorded deviations, the complete twenty-item ledger with dispositions, and the five-step post-merge runbook that tags and archives this release: `docs/devcontainer-upgrade/stage-11/README.md`.
